...
savepath1      <- paste0("D:/experiment/Conference Paper/ECML/ECML2022/ECML2022 Result/",
                         paste0("IOKS-logistic-",Dataset[d_index],".txt"))
savepath2      <- paste0("D:/experiment/Conference Paper/ECML/ECML2022/ECML2022 Result/",
                         paste0("IOKS-logistic-all-",Dataset[d_index],".txt"))
...
for(re in 1:reptimes)
{
  ...
  q        <- p
#  eta0     <- 4*len_sigma^(1/3)*l_max/(G_1*U*sqrt(length_tr)*(log(length_tr))^(2/3)) 
  ...
  for (i in 1:length_tr)
  {
    ...
    norm_tilde_nabla[It] <- norm_tilde_nabla[It]+(tem1/(1+tem1))^2/(p[It])^2
    ...
    if(p[It]>max(eta))
      tilde_ell[It] <- loss/l_max/p[It]
    else
      tilde_ell[It] <- loss/l_max/(p[It]+max(eta))
